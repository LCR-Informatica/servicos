...
if ($id > 0) {
    ...
    if (!$erro && $del) {
        ...
        // LOG
        funcoes::CriaLOG($_SESSION['nome_usuario'], ' apagou o servico no BD');
        $sucesso = true;
    }
} else {
    ...
    <div class="container-fluid perfil">
        ...
        <div class="row justify-content-center">
            <div class="col-md-8 card text-center m-3">
                ...
                <h5 class="mt-3">Descriçao: <?= $servico['descricao']?></h5>
                <h5>Tipo: <?= ($servico['orcamento']) ? 'Serviço' : 'Orçamento' ?></h5>
                <h5>Valor: <?= $servico['valor']?></h5>
                <h5>Quantidade: <?= $servico['quantidade']?></h5>
                <div class="pt-3 pb-3">
                    <a class="mr-5 btn btn-primary btn-size-150" 
                    href="?a=servicos-del&s=<?= $servico['id_servico']?>&d=true">Excluir</a>
                    <a class="btn btn-secondary btn-size-150" 
                    href="?a=servicos-list&i=<?= $cliente_atual?>">
                    Voltar</a>
                </div>
            </div>
        </div>
