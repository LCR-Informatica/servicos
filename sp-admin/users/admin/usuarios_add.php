...
if ($permissao) {
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        if (!$erro) {
            ...
            $mailTo = new cl_Email();
            $mailTo->EnviarEmailAdmin($dados_email);

            if (!$mailTo) {
                $erro = true;
                $msg = 'Erro no envio do email para o usuário! Favor tentar novamente mais tarde!';
            }
        }
                    ...
                    <form action="?a=usuarios-add" method="post">
                        <div class="col-md-10 offset-md-1 mt-1 pb-1">
                            <div class='row ml-2 form-group'>
                                ...
                                <input class='col-sm-8 form-control' type="text" name="txt-nome" required placeholder="Digite o nome do usuário" pattern=".{4,50}" title="Entre 5 e 50 carcateres.">
                            </div>

                            <div class='row ml-2 form-group'>
                                <label class="col-sm-2 col-form-label"><strong>Email:</strong></label>
                                <input class='col-sm-8 form-control' type="email" name="txt-email" required placeholder="Digite o email do usuário" pattern=".{4,50}" title="Entre 5 e 50 carcateres.">
                            </div>

                            <div class='row ml-2 form-group'>
                                <label class="col-sm-2 col-form-label"><strong>Usuário:</strong></label>
                                <input class='col-sm-8 form-control' type="text" name="txt-usuario" required placeholder="Digite a identificação do usuário" pattern=".{4,50}" title="Entre 5 e 50 carcateres.">
                            </div>

                            <div class='row ml-2 form-group'>
                                <label class="col-sm-2 col-form-label"><strong>Senha:</strong></label>
                                <input class='col-sm-8 form-control' type="password" name="txt-senha" id='txt_senha' required readonly placeholder="Senha do usuário">
                                <button type="button" class="btn btn-primary ml-2" onclick="gerarSenha(15)">Gerar Senha</button>
                            </div>
                        </div>
                        <hr>
                        <div class="text-center pb-1">
                            <a class="btn btn-secondary btn-size-150 mr-2" href="?a=usuarios-gerir">Voltar</a>
                            <button type="submit" class="btn btn-primary btn-size-150 mr-2">Incluir</button>
                            <button type="button" class="btn btn-info btn-size-250" data-toggle="collapse" data-target='#caixa_permissoes'>Definir Permissões</button>
                        </div>
                            ...
                            <div class="row m-3">
                                ...
                                ?>
                                    ...
                                    <div class="col-sm-4 checkbox">
                                        <input type="checkbox" name="check-permissoes[]" id="checkbox-permissao" value='<?php echo $id; ?>'>
